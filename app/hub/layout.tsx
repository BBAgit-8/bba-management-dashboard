...
export default function HubLayout({ children }: { children: React.ReactNode }) {
  ...
  // Don't render layout on the login page itself
    ...
    <div className="flex h-screen overflow-hidden bg-slate-50">
      ...
      <aside className="fixed inset-y-0 left-0 z-40 flex w-60 flex-col bg-purple-950 border-r border-white/10">
        <div className="flex h-16 items-center gap-3 px-5 border-b border-white/10">
          ...
          <span className="text-sm font-semibold text-white/90">BBA Team Hub</span>
        </div>
            ...
            return (
              <Link key={item.href} href={item.href}
                className={`group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${
                  active ? 'bg-white/15 text-purple-300 ring-1 ring-inset ring-purple-400/40'
                         : 'text-white/70 hover:bg-white/10 hover:text-white'}`}>
                <span className={active ? 'text-purple-300' : 'text-white/40 group-hover:text-white/70'}>{item.icon}</span>
                {item.label}
        ...
        <div className="border-t border-white/10 px-3 py-4 space-y-1">
          <div className="flex items-center gap-3 px-3 py-2">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-[10px] text-white/50">Team Member</p>
            </div>
          </div>
